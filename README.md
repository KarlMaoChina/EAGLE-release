...
This repository contains the public methods code. It does not include patient images, case identifiers, site-specific tables, or trained weights.
...
Random seed for fold use, initialization, augmentation, and bootstrap resampling is `42`. Locked numeric constants live in `eagle/spec.py` and are mirrored in `configs/eagle_v1.yaml`.
...
scripts/               paper-constant check and synthetic smoke
...
Case tables use a generic `case_id` column. Do not put identifiable source codes in that field if you share tables. Spacing tuples follow the NIfTI data-array axes returned by nibabel.

## Install

Python 3.10+ and a working PyTorch install are required.
...
`infer` expects `checkpoints/preprocess_stats.json` and `checkpoints/biamf/fold_{0-4}.pt`. Weights are not shipped here; place a freeze package in that layout. `extract-radiomics` needs `pip install -e ".[radiomics]"`.
...
`check_spec.py` compares frozen constants with Appendix 1 / Methods (clinical names, 32 radiomic names, geometry, optimizer settings, T = 0.5). `smoke_synthetic.py` runs mask cleanup, dual-view patches, fit-stats and five-fold inference on a random-init ensemble. Random-init probabilities are not performance numbers.

## Scope of this release

- The public CLI takes a gallbladder mask and applies the anatomy-aware cleanup. The nnU-Net `3d_cascade_fullres` segmentation trainer is not bundled.
- DVSE streams are trained one view at a time. The manuscript alternating joint trainer is not in this package.
- The image encoder is a self-contained ResNet-18-3D with spatial prior modulation (204-d pooled vector at channel scale 0.4). Laboratory checkpoints that used a different module layout will not load by state-dict key name.
- Intensity windowing follows Appendix 1: window level 40 HU, window width 300 HU.
